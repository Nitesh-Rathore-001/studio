...
export default function ClassicOnlineGamePage() {
  ...
  const boardArray = useMemo(() => {
    ...
    return game?.size ? createEmptyBoard(game.size) : [];
  }, [game]);
  ...
  const handleCellClick = async (row: number, col: number) => {
    ...
    const newBoard = { ...game.board, [`${row}-${col}`]: game.currentPlayer };
    const newBoardArray = boardMapToArray(newBoard, game.size);
    
    const winInfo = checkWin(newBoardArray, game.currentPlayer, game.winCondition);
    const isDraw = !winInfo && Object.keys(newBoard).length === game.size * game.size;

    const nextPlayer = game.currentPlayer === "X" ? "O" : "X";
    const newStatus = winInfo ? "finished" : isDraw ? "finished" : "playing";

    try {
        await updateDoc(doc(db, "classicGames", gameId), {
            board: newBoard,
            currentPlayer: nextPlayer,
            winner: winInfo ? game.currentPlayer : isDraw ? "D" : null,
            ...
            {game.status === 'waiting' && (
             ...
             {game.status !== 'waiting' && (
                <div className="text-xl">
                    {game.winner && game.winner !== 'D' ? `Player ${game.winner} Wins!` : isDraw ? "It's a Draw!" : `Turn: ${game.currentPlayer}`}
                </div>
             )}
